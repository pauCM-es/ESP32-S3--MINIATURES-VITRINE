...
class ModeManager {
public:
    ModeManager(LedMovementControl& ledMovementControl, NFCReaderControl& nfcReader, TFTDisplayControl& displayControl);

    void addNewMiniature();
    void setStandbyBrightness(uint8_t brightness);
    void selectMode(const char* options[], int numOptions, std::function<void(int)> callback);
